...
class MyCustomLayer(layers.Layer):
    def __init__(self, weight_matrix, predicted_labels, **kwargs):
    ...
    def call(self, inputs):
        ...
        weights = tf.expand_dims(tf.expand_dims(self.weight_matrix, axis=0), axis=-1)
        ...
        similarity = tf.keras.losses.CosineSimilarity(axis=1)(inputs, weight_submatrices)
        ...
        predicted_label = self.predicted_labels[
                                                argmax_index // self.predicted_labels.shape[0],
                                                argmax_index % self.predicted_labels.shape[1]]
...
if __name__ == '__main__':
    ...
    inference_layer = MyCustomLayer(weight_matrix=som, 
    ...
    with tf.keras.utils.custom_object_scope(custom_objects):
        ...
        print("Model compatible for Akida conversion:", check_model_compatibility(new_model, input_is_image=False))
        akida_model = convert(new_model, 
